...
{#snippet NavItem(label: string, sublabels: string[])}
  <li class="flex flex-col items-center gap-[1.8125rem] lg:items-start">
    <a
      class="relative font-bold uppercase transition-shadow before:absolute before:top-full before:right-0 before:left-0 before:border-b-2 before:border-transparent before:transition-colors hover:before:border-white"
      href="/">{label}</a
    >
    <ul class="flex flex-col gap-[1.0625rem]">
      {#each sublabels as sublabel}
        <li class="tracking-tighter text-gray-500">
          <a class="transition-colors hover:text-blue-400" href="/">{sublabel}</a>
        </li>
      {/each}
    </ul>
  </li>
...
<footer class="bg-gray-950">
  <Container>
    <div
      class="flex flex-col items-center gap-12 py-14 text-base leading-none text-white lg:flex-row lg:items-start lg:pt-[4.4375rem] lg:pb-[4.6875rem]"
    >
      <a class="group" href="/" aria-current="page" aria-label="Homepage">
        <svg xmlns="http://www.w3.org/2000/svg" width="121" height="33"
          ><path
            class="fill-white transition-colors group-hover:fill-blue-400"
            fill="#34313D"
      ...
      <nav class="w-full text-center lg:pt-1 lg:pl-50 lg:text-start" aria-label="Footer navigation">
        <ul class="grid gap-12 md:grid-cols-3">
          ...
          {@render NavItem("Company", ["About", "Our Team", "Careers", "Contact"])}
        </ul>
      </nav>
      <ul class="flex gap-6 [&_a]:transition-colors [&_a]:hover:text-blue-400">
        <li>
          <a href="/" aria-label="Facbook">
